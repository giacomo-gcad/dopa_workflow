...
ALTER TABLE :vSCHEMA.wdoecm_geom_:vDATE
--DROP COLUMN geom_was_invalid,
ADD COLUMN area_geo double precision;

UPDATE :vSCHEMA.wdoecm_geom_:vDATE
SET area_geo = (ST_AREA(geom::geography)/1000000);

-- THIRD PART: Create final tables

DROP TABLE IF EXISTS :vSCHEMA.wdoecm_:vDATE;
CREATE TABLE :vSCHEMA.wdoecm_:vDATE AS
WITH
geoms AS (SELECT * FROM :vSCHEMA.wdoecm_geom_:vDATE),
...
ALTER TABLE :vSCHEMA.wdoecm_:vDATE
ADD PRIMARY KEY (wdpaid),
DROP COLUMN ogc_fid;
CREATE INDEX wdoecm_:vDATE:vidx ON :vSCHEMA.wdoecm_:vDATE USING gist(geom);
