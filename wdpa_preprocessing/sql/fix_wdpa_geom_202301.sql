...
WITH
a AS (SELECT wdpaid FROM protected_sites.wdpa_wdoecm_geom_202301 WHERE geom_was_invalid IS TRUE),
b AS (SELECT wdpaid,(ST_Dump(ST_Multi(shape))).* FROM  protected_sites.wdpa_wdoecm_poly_input_202301 WHERE wdpaid IN (SELECT wdpaid FROM a)),
...
UPDATE protected_sites.wdpa_wdoecm_geom_202301
SET geom=b.geom
FROM (SELECT * FROM wdpa_correction) b
WHERE wdpa_wdoecm_geom_202301.wdpaid=b.wdpaid; -- check before that the original geom doesn't contain further geometries (the check is coming from a dump)
