...
WITH
a AS (SELECT wdpaid FROM protected_sites.wdpa_geom_202003 WHERE geom_was_invalid IS TRUE),
b AS (SELECT wdpaid,(ST_Dump(ST_Multi(wkb_geometry))).* FROM  protected_sites.wdpa_poly_input_202003 WHERE wdpaid IN (SELECT wdpaid FROM a)),
...
UPDATE protected_sites.wdpa_202003
SET geom=b.geom
FROM (SELECT * FROM wdpa_correction) b
WHERE wdpa_202003.wdpaid=b.wdpaid; -- check before that the original geom doesn't contain further geometries (the check is coming from a dump)
