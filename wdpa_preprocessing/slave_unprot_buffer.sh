...
r.mask --quiet --overwrite vector=${BU}
r.mapcalc expression=\"be_${BU}=(if(isnull(${FLAT}),1,null()))*MASK\" --overwrite --quiet
#g.remove type=vector name=${BU} -f
r.to.vect --quiet --overwrite -t input=be_${BU} output=${BU} type=area
