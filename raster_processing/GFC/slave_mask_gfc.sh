...
for FIL in $(cat ${TREE_LIST_FILE}|awk "NR >= (${TIL}+1) && NR <= (${TIL}+${TILESIZE})")
	...
	mask=${temp_dir}/mask_${FIL:36:48}
	tr_out2=${tree_outdir_ll}/${FIL}
	ga_inp=${gain_indir}/${rootstring}"gain_"${FIL:36:48}
	ga_out1=${gain_outdir_ll}/${rootstring}"gain_"${FIL:36:48}
    echo "
...
for FIL in $(cat ${LOSS_LIST_FILE}|awk "NR >= (${TIL}+1) && NR <= (${TIL}+${TILESIZE})")
    do
	ly_inp=${lossyear_indir}/${rootstring}"lossyear_"${FIL:31:43}
	mask=${temp_dir}/mask_${FIL:31:43}
	ly_out1=${lossyear_outdir_ll}/${rootstring}"lossyear_"${FIL:31:43}
    echo "
...
"> ./dyn/mask_loss_${FIL:31:39}.sh
    chmod u+x ./dyn/mask_loss_${FIL:31:35}.sh
    ./dyn/mask_loss_${FIL:31:35}.sh
done
