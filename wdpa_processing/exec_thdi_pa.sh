...
# echo " "
# echo "This script executes in parallel python scripts for computation of Terrestrial Habitat Diversity Index (THDI)"
# echo "It works only if it is run from a dedicated docker container with GRASS 7.1 installed"
# echo " "
# echo "Refer to https://github.com/javimarlop/eHabpy for more details"
# echo " "
# read -p "Are you running the script from a dedicated Docker container? (y/n) " -n 1 -r
# echo " "
...
# then
	...
	for TIL in $(for i in $(eval echo {0..$NTILES}); do ((start=${TILESIZE}*$i)); echo -n $start" "; done)
		...
		start=${end}
		python slave_thdi_pa.py ${first_pa} ${last_pa} ${msn} ${respath} ${terr_pas} ${grassdb} ${grassloc} &
	done
