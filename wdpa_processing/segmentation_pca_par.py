...
NPROC = int(os.environ.get('EHAB_NPROC', '1'))  # set to 1 to avoid double multiprocessing
...
##### block added from old script  #####
mapset = 'map' + sys.argv[3]
col= 'cat'
shp_path = sys.argv[4] + '/shp/'
csv_path = sys.argv[4] + '/csv/'
...
pa_list2 = np.genfromtxt(pa_tc_list,dtype=str)
...
def fsegm(pa):
	...
	if pa not in pa_list_done:
		...
		os.system('rm -rf ' + os.path.join(GRASSDBASE, MYLOC, mapset))
		col = 'wdpaid'
		grass.run_command('g.mapset', mapset=mapset, location=MYLOC, dbase=GRASSDBASE, flags='c')
		os.system('rm '+csv_path+'*_' + str(pa) + '_*')
		os.system('rm '+shp_path+'*_' + str(pa) + '_*')
...
########## SIAMO QUI
		...
		try:
			gsetup.init(GRASSDBASE, MYLOC, mapset)
		except TypeError:
			gsetup.init(os.environ['GISBASE'], GRASSDBASE, MYLOC, mapset)
		print(pa, mapset, grass.gisenv())
		# ong = str(pa) + str(mapset) + str(grass.gisenv())
		# grass.run_command('g.mapsets', mapset='ehabplus_cs,rasterized_parks,PERMANENT,WDPA_PA', operation='add')
		...
		# os.system('cat ' + os.path.join(GRASSDBASE, MYLOC, mapset, 'group/segm/REF'))
		# j = 0
		# for thr in reps:
			# pa2 = pa + 'v2_' + str(j)
			# pa2s = pa + 'v2_' + str(j - 1)
			...
			# name = 'shp/park_segm_' + str(pa) + '_' + str(j)+ '_' + sys.argv[3]
