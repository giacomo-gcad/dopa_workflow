...
# merge results
cat ${RESULTSPATH}/bu_groads_parte_*.csv >> ${RESULTSPATH}/bu_groads.csv
wait

## delete mapsets and intermediate results
for TIL in $(for i in $(eval echo {0..$NTILES}); do ((start=${TILESIZE}*$i)); echo -n $start" "; done)
	do
	rm -rf ${LOCATION_MO_PATH}/groads_${TIL}
	rm -f ${RESULTSPATH}/bu_groads_parte*.csv
done
...
for BU in $(cat ${MISSING_BU_LIST})
	do
	grass ${PERMANENT_MO_MAPSET} --exec ./dyn/process_groads_${BU}.sh
done
cat ${RESULTSPATH}/bu_groads_parte_*.csv >> ${RESULTSPATH}/bu_groads.csv
...
for TIL in $(for i in $(eval echo {0..$NTILES}); do ((start=${TILESIZE}*$i)); echo -n $start" "; done)
	do
	rm -f ${RESULTSPATH}/bu_groads_parte${TIL}.csv
done
rm -f ./*actually_done.csv
rm -f ./*to_be_repeated.csv

## delete dynamic scripts
echo dyn/process_groads_*.sh |xargs rm -f
...
echo "---------------------------------------------------------------------------------------"
echo "Stats on PAs and Roads computed in "${runtime}" minutes using "${NCORES}" cores "
echo "---------------------------------------------------------------------------------------"
exit
