...
## PART I: FIND COMMON TILES IN BOTH THEMES (PABU AND BUILTUP)
ls -1 ${DATABASE}/${LOCATION_LL}/${IN_RASTER_MAPSET}"/cell_misc"| sed 's/builtup_//' >builtup_tiles.txt
...
do	
	...
	echo "./slave_cep_catraster_stats.sh ${eid} ${TMP_MAPSET_PATH} ${RESULTSPATH} ${IN_RASTER}_${eid}@${IN_RASTER_MAPSET} ${OUTCSV} ${PABU_MAPSET}"
