...
IN_RASTER="gfc_treecover_over30"
IN_RASTER_ROOT="treecover_tile_"
...
echo "now running r.univar in parallel on 504 CEP tiles and "${IN_RASTER}" using ${NCORES} threads"
...
do	
	...
	wait
	echo "./slave_cep_conraster_stats.sh ${eid} ${TMP_MAPSET_PATH} ${RESULTSPATH_TMP} ${IN_RASTER_ROOT}${eid}@${IN_RASTER_MAPSET} ${OUTCSV} ${CEP_MAPSET}"
...
echo "---------------------------------------------------------------------------------------"
echo "Stats on CEP and GFC TREECOVER computed in "${runtime}" minutes using "${NCORES}" cores "
echo "---------------------------------------------------------------------------------------"
exit
