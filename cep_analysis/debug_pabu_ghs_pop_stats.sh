...
do
	...
	OUTCSV_ROOT="prova_pabu_"${IN_RASTER_ROOT}
	FINALCSV="prova_r_univar_"${OUTCSV_ROOT}${wdpadate}
	
	#for eid in $(cat pabu_tiles.txt)
	for eid in 318 336 339 340 373 378 381 382 404 405 406 622 623 624 625 626
	do	
		...
		wait
		echo "./slave_debug_conraster_stats.sh ${eid} ${TMP_MAPSET_PATH} ${RESULTSPATH} ${IN_RASTER}@${IN_RASTER_MAPSET} ${OUTCSV} ${PABU_MAPSET}"
...
rm -f ${RESULTSPATH}/prova_pabu*.csv
