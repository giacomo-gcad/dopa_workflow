--COUNTRY
DROP TABLE IF EXISTS results_202601_cep_out.country_lc_copernicus_array;CREATE TABLE results_202601_cep_out.country_lc_copernicus_array AS 
WITH
i AS (SELECT * FROM results_202601_cep_out.country_intermediate_lc_copernicus ORDER BY country_id,cat),
tot_ext AS(
SELECT
country_id,
...
/* --ECO
DROP TABLE IF EXISTS results_202601_cep_out.ecoregion_lc_copernicus;CREATE TABLE results_202601_cep_out.ecoregion_lc_copernicus AS 
WITH
i AS (SELECT * FROM results_202601_cep_out.ecoregion_intermediate_lc_copernicus ORDER BY eco_id,cat),
tot_ext AS(
SELECT
eco_id,
ARRAY_AGG(cat) lc_copernicus_code,
...
--PA
DROP TABLE IF EXISTS results_202601_cep_out.wdpa_lc_copernicus;CREATE TABLE results_202601_cep_out.wdpa_lc_copernicus AS 
WITH
i AS (SELECT * FROM results_202601_cep_out.wdpa_intermediate_lc_copernicus ORDER BY wdpaid,cat),
tot_ext AS(
SELECT
wdpaid,
ARRAY_AGG(cat) lc_copernicus_code,
