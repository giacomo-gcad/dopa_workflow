...
SELECT country,SUM(sqkm) country_tot_sqkm FROM :v_rcep_in.index_country_cep_last GROUP BY country ORDER BY country;
DROP TABLE IF EXISTS t_country_land;CREATE TEMPORARY TABLE t_country_land AS
SELECT country,SUM(sqkm) country_land_sqkm FROM :v_rcep_in.index_country_cep_last WHERE is_marine IS FALSE GROUP BY country ORDER BY country;
DROP TABLE IF EXISTS t_country_marine;CREATE TEMPORARY TABLE t_country_marine AS
SELECT country,SUM(sqkm) country_marine_sqkm FROM :v_rcep_in.index_country_cep_last WHERE is_marine IS TRUE GROUP BY country ORDER BY country;
DROP TABLE IF EXISTS t_country_prot ;CREATE TEMPORARY TABLE t_country_prot AS
SELECT country,SUM(sqkm) country_prot_sqkm FROM :v_rcep_in.index_country_cep_last WHERE is_protected IS TRUE GROUP BY country ORDER BY country;
DROP TABLE IF EXISTS t_country_land_prot;CREATE TEMPORARY TABLE t_country_land_prot AS
SELECT country,SUM(sqkm) country_land_prot_sqkm FROM :v_rcep_in.index_country_cep_last WHERE is_marine IS FALSE AND is_protected IS TRUE GROUP BY country ORDER BY country;
DROP TABLE IF EXISTS t_country_marine_prot;CREATE TEMPORARY TABLE t_country_marine_prot AS
SELECT country,SUM(sqkm) country_marine_prot_sqkm FROM :v_rcep_in.index_country_cep_last WHERE is_marine IS TRUE AND is_protected IS TRUE GROUP BY country ORDER BY country;
---------------------------------------------------------------------------------------
-- ecoregion_block
---------------------------------------------------------------------------------------
DROP TABLE IF EXISTS t_ecoregion_tot;CREATE TEMPORARY TABLE t_ecoregion_tot AS
SELECT ecoregion,SUM(sqkm) ecoregion_tot_sqkm FROM :v_rcep_in.index_ecoregion_cep_last GROUP BY ecoregion ORDER BY ecoregion;
DROP TABLE IF EXISTS t_ecoregion_prot;CREATE TEMPORARY TABLE t_ecoregion_prot AS
SELECT ecoregion,SUM(sqkm) ecoregion_prot_sqkm FROM :v_rcep_in.index_ecoregion_cep_last WHERE is_protected IS TRUE GROUP BY ecoregion ORDER BY ecoregion;
---------------------------------------------------------------------------------------
...
WITH a AS (SELECT * FROM :v_rcep_in.index_country_cep_last a NATURAL JOIN :v_rcep_in.index_ecoregion_cep_last b)
...
LEFT JOIN :v_rcep_in.atts_ecoregion_last c USING(ecoregion)
...
FROM :v_rcep_in.atts_ecoregion_last a
...
SELECT pa,SUM(sqkm) pa_rtot_sqkm FROM :v_rcep_in.index_pa_cep_last GROUP BY pa ORDER BY pa;
...
FROM :v_rcep_in.atts_pa_last a ORDER BY wdpaid;
