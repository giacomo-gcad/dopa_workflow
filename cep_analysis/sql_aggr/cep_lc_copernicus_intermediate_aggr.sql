...
-- INPUTS
-----------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS theme; CREATE TEMPORARY TABLE theme AS SELECT * FROM results_202601_cep_in.r_stats_cep_copernicus_lc_2019_202601;
DROP TABLE IF EXISTS country_index;CREATE TEMPORARY TABLE country_index AS SELECT * FROM cep_data_202601.cep_index;
--DROP TABLE IF EXISTS ecoregion_index; CREATE TEMPORARY TABLE ecoregion_index AS SELECT * FROM results_202601_cep_in.index_ecoregion_cep_last;
...
t_land AS (SELECT country_id,cat,ROUND(SUM(sqkm::numeric),2) theme_land_sqkm FROM a WHERE is_marine IS FALSE GROUP BY country_id,cat),
t_land_prot AS (SELECT country_id,cat,ROUND(SUM(sqkm::numeric),2) theme_land_prot_sqkm FROM a WHERE is_marine IS FALSE AND is_protected IS TRUE GROUP BY country_id,cat),
...
land AS (SELECT country_id,ROUND(SUM(sqkm::numeric),2) land_sqkm FROM a WHERE is_marine IS FALSE GROUP BY country_id),
land_prot AS (SELECT country_id,ROUND(SUM(sqkm::numeric),2) land_prot_sqkm FROM a WHERE is_marine IS FALSE AND is_protected IS TRUE GROUP BY country_id),
...
t_land AS (SELECT eco_id,cat,ROUND(SUM(sqkm::numeric),2) theme_land_sqkm FROM a WHERE is_marine IS FALSE GROUP BY eco_id,cat),
t_land_prot AS (SELECT eco_id,cat,ROUND(SUM(sqkm::numeric),2) theme_land_prot_sqkm FROM a WHERE is_marine IS FALSE AND is_protected IS TRUE GROUP BY eco_id,cat),
...
land AS (SELECT eco_id,ROUND(SUM(sqkm::numeric),2) land_sqkm FROM a WHERE is_marine IS FALSE GROUP BY eco_id),
land_prot AS (SELECT eco_id,ROUND(SUM(sqkm::numeric),2) land_prot_sqkm FROM a WHERE is_marine IS FALSE AND is_protected IS TRUE GROUP BY eco_id),
...
-- COUNTRY OUTPUT
-----------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS results_202601_cep_out.country_intermediate_lc_copernicus;
CREATE TABLE results_202601_cep_out.country_intermediate_lc_copernicus AS
SELECT * FROM country_theme;
/* -----------------------------------------------------------------------------------------------------------------------------
-- ECOREGION OUTPUT
-----------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS results_202601_cep_out.ecoregion_intermediate_lc_copernicus;
CREATE TABLE results_202601_cep_out.ecoregion_intermediate_lc_copernicus AS
SELECT * FROM ecoregion_theme; */
-----------------------------------------------------------------------------------------------------------------------------
-- PA OUTPUT
-----------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS results_202601_cep_out.wdpa_intermediate_lc_copernicus;
CREATE TABLE results_202601_cep_out.wdpa_intermediate_lc_copernicus AS
