-- in 247p
--CREATE TABLE cep_data_202601.cep_index AS SELECT * FROM cep_out.index_cep;

DROP TABLE IF EXISTS country_cid;CREATE TEMPORARY TABLE country_cid AS
SELECT DISTINCT qid,cid,country_id,is_protected,is_marine FROM cep_data_202601.cep_index
WHERE is_marine IS FALSE  ORDER BY cid,country_id;

DROP TABLE IF EXISTS pa_cid;CREATE TEMPORARY TABLE pa_cid AS
SELECT DISTINCT cid,pa,is_protected FROM cep_data_202601.cep_index
WHERE is_protected IS TRUE ORDER BY cid,pa;


--COUNTRY
DROP TABLE IF EXISTS results_202601_cep_out.country_lcc_esa;CREATE TABLE results_202601_cep_out.country_lcc_esa AS
WITH
a AS (SELECT DISTINCT cat,lc1_1995,lc1_2020 FROM results_202601_cep_in.r_stats_cep_esa_lcc_1995_2020_202601 ORDER BY cat),
b AS (SELECT
...
FROM a JOIN results_202601_cep_out.country_intermediate_lcc_esa USING (cat) ORDER BY country_id,cat),
...
/* --ECOREGION
DROP TABLE IF EXISTS results_202601_cep_out.ecoregion_lcc_esa;CREATE TABLE results_202601_cep_out.ecoregion_lcc_esa AS 
WITH
...
--	  theme_land_sqkm esa_lcc_sqkm,theme_land_prot_sqkm esa_lcc_prot_sqkm	  
	  ...
	  FROM a
	  JOIN results_202601_cep_out.ecoregion_intermediate_lcc_esa USING (cat)
	  ORDER BY eco_id,cat
),
...
--PA
DROP TABLE IF EXISTS results_202601_cep_out.wdpa_lcc_esa; CREATE TABLE results_202601_cep_out.wdpa_lcc_esa AS 
WITH
a AS (SELECT DISTINCT cat,lc1_1995,lc1_2020 FROM results_202601_cep_in.r_stats_cep_esa_lcc_1995_2020_202601 ORDER BY cat),
b AS (SELECT
	  wdpaid,cat,lc1_1995,lc1_2020,tot_sqkm lcc_esa_pa_tot_sqkm,theme_sqkm lcc_esa_sqkm
	  FROM a JOIN results_202601_cep_out.wdpa_intermediate_lcc_esa USING (cat) ORDER BY wdpaid,cat),
c AS (SELECT
	wdpaid,
	ARRAY_AGG(lc1_1995)lcc_esa_lc1_1995,
	ARRAY_AGG(lc1_2020)lcc_esa_lc1_2020,
