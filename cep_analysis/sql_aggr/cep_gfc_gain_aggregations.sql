...
SELECT * FROM results_202601_cep_in.r_stats_cep_gfc_gain_over30_202601;
...
WHERE is_marine IS FALSE GROUP BY country_id,is_marine,is_protected,cid;
...
WITH
a AS (SELECT country_id,tot_sqkm,tot_prot_sqkm,SUM(cat_sqkm) cat_sqkm,SUM(cat_prot_sqkm) cat_prot_sqkm FROM country_theme WHERE cat = 1 GROUP BY country_id,tot_sqkm,tot_prot_sqkm)
SELECT
country_id,
...
--------------------------------------------
-- country
DROP TABLE IF EXISTS results_202601_cep_out.country_global_forest_cover_gain; CREATE TABLE results_202601_cep_out.country_global_forest_cover_gain AS
SELECT * FROM country_change_in_forest_cover_gain;
/* -- ecoregion
DROP TABLE IF EXISTS results_202601_cep_out.ecoregion_global_forest_cover_gain; CREATE TABLE results_202601_cep_out.ecoregion_global_forest_cover_gain AS
SELECT * FROM ecoregion_change_in_forest_cover_gain; */
-- protection
DROP TABLE IF EXISTS results_202601_cep_out.wdpa_global_forest_cover_gain; CREATE TABLE results_202601_cep_out.wdpa_global_forest_cover_gain AS
