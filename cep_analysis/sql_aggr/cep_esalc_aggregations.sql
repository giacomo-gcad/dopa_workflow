DROP TABLE IF EXISTS results_202601_cep_out.wdpa_lc_esa CASCADE;

DROP TABLE IF EXISTS abc;CREATE TEMPORARY TABLE abc AS
WITH
a AS (SELECT DISTINCT wdpaid FROM protected_sites.wdpa_wdoecm_202601),
b AS (SELECT lc_code lc_esa_code FROM themes.class_lc_esa)
SELECT * FROM a,b;

DROP TABLE IF EXISTS lc_1995;CREATE TEMPORARY TABLE lc_1995 AS
SELECT wdpaid,cat lc_esa_code,ROUND(theme_sqkm::numeric,2) lc_esa_1995_sqkm FROM results_202601_cep_out.wdpa_intermediate_lc_esa_1995 ORDER BY wdpaid,lc_esa_code;

DROP TABLE IF EXISTS lc_2000;CREATE TEMPORARY TABLE lc_2000 AS
SELECT wdpaid,cat lc_esa_code,ROUND(theme_sqkm::numeric,2) lc_esa_2000_sqkm FROM results_202601_cep_out.wdpa_intermediate_lc_esa_2000 ORDER BY wdpaid,lc_esa_code;

DROP TABLE IF EXISTS lc_2005;CREATE TEMPORARY TABLE lc_2005 AS
SELECT wdpaid,cat lc_esa_code,ROUND(theme_sqkm::numeric,2) lc_esa_2005_sqkm FROM results_202601_cep_out.wdpa_intermediate_lc_esa_2005 ORDER BY wdpaid,lc_esa_code;

DROP TABLE IF EXISTS lc_2010;CREATE TEMPORARY TABLE lc_2010 AS
SELECT wdpaid,cat lc_esa_code,ROUND(theme_sqkm::numeric,2) lc_esa_2010_sqkm FROM results_202601_cep_out.wdpa_intermediate_lc_esa_2010 ORDER BY wdpaid,lc_esa_code;

DROP TABLE IF EXISTS lc_2015;CREATE TEMPORARY TABLE lc_2015 AS
SELECT wdpaid,cat lc_esa_code,ROUND(theme_sqkm::numeric,2) lc_esa_2015_sqkm FROM results_202601_cep_out.wdpa_intermediate_lc_esa_2015 ORDER BY wdpaid,lc_esa_code;

DROP TABLE IF EXISTS lc_2020;CREATE TEMPORARY TABLE lc_2020 AS
SELECT wdpaid,cat lc_esa_code,ROUND(theme_sqkm::numeric,2) lc_esa_2020_sqkm FROM results_202601_cep_out.wdpa_intermediate_lc_esa_2020 ORDER BY wdpaid,lc_esa_code;
...
DROP TABLE IF EXISTS results_202601_cep_out.wdpa_lc_esa; CREATE TABLE results_202601_cep_out.wdpa_lc_esa AS SELECT * FROM f;