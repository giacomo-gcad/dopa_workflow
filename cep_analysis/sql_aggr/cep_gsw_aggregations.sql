...
SELECT * FROM results_202601_cep_in.r_stats_cep_gsw_transitions_2021_202601;
--------------------------------------------------------------
...
WITH
a as (SELECT * FROM country_index a JOIN cep_theme b USING(cid) WHERE is_marine IS NOT TRUE),
b AS (SELECT country_id,SUM(sqkm) tot_sqkm FROM a GROUP BY country_id),
c AS (SELECT country_id,cat,SUM(cat_sqkm) sqkm FROM a GROUP BY country_id,cat),
d AS (SELECT country_id,cat,SUM(cat_sqkm) prot_sqkm FROM a WHERE is_protected IS TRUE GROUP BY country_id,cat)
SELECT *
FROM b
LEFT JOIN c USING(country_id)
LEFT JOIN d USING(country_id,cat)
...
WITH
list AS (SELECT DISTINCT country_id FROM country_theme),
--Permanent water by country  (tot and prot)
perm2018 AS (SELECT country_id, SUM(sqkm) perm2018 FROM country_theme where cat=1 OR cat=2 OR cat=7 GROUP BY country_id),
perm1984 AS (SELECT country_id, SUM(sqkm) perm1984 FROM country_theme where cat=1 OR cat=3 OR cat=8 GROUP BY country_id),
perm2018_prot AS (SELECT country_id, SUM(prot_sqkm) perm2018_prot FROM country_theme where cat=1 OR cat=2 OR cat=7 GROUP BY country_id),
perm1984_prot AS (SELECT country_id, SUM(prot_sqkm) perm1984_prot FROM country_theme where cat=1 OR cat=3 OR cat=8 GROUP BY country_id),
--Seasonal water by country (tot and prot)
seas2018 AS (SELECT country_id, SUM(sqkm) seas2018 FROM country_theme where cat=4 OR cat=5 OR cat=8 GROUP BY country_id),
seas1984 AS (SELECT country_id, SUM(sqkm) seas1984 FROM country_theme where cat=4 OR cat=6 OR cat=7 GROUP BY country_id),
seas2018_prot AS (SELECT country_id, SUM(prot_sqkm) seas2018_prot FROM country_theme where cat=4 OR cat=5 OR cat=8 GROUP BY country_id),
seas1984_prot AS (SELECT country_id, SUM(prot_sqkm) seas1984_prot FROM country_theme where cat=4 OR cat=6 OR cat=7 GROUP BY country_id)
...
FROM
list z
LEFT JOIN perm2018 a USING(country_id) 
LEFT JOIN perm1984 b USING(country_id)
LEFT JOIN seas2018 e USING(country_id)
LEFT JOIN seas1984 f USING(country_id)
LEFT JOIN perm2018_prot c USING(country_id)
LEFT JOIN perm1984_prot d USING(country_id)
LEFT JOIN seas2018_prot g USING(country_id)
LEFT JOIN seas1984_prot h USING(country_id)
ORDER BY z.country_id;
...
DROP TABLE IF EXISTS results_202601_cep_out.country_water_surface_inland;
CREATE TABLE results_202601_cep_out.country_water_surface_inland AS
SELECT * FROM cep_country_gsw_output;

-- DROP TABLE IF EXISTS results_202601_cep_out.ecoregion_water_surface_inland;
-- CREATE TABLE results_202601_cep_out.ecoregion_water_surface_inland AS
-- SELECT * FROM cep_eco_gsw_output WHERE eco_id IN (SELECT ecoregion FROM ecoregion_index WHERE is_marine IS FALSE);

DROP TABLE IF EXISTS results_202601_cep_out.wdpa_water_surface_inland;
CREATE TABLE results_202601_cep_out.wdpa_water_surface_inland AS
