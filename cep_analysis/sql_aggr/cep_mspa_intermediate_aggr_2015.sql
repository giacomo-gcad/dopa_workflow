...
-- INPUTS
-----------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS theme15; CREATE TEMPORARY TABLE theme15 AS SELECT * FROM results_202601_cep_in.r_stats_cep_mspa_lc_2015_202601;
...
-- PRE-PROCESSING
-----------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS theme15_sqkm; CREATE TEMPORARY TABLE theme15_sqkm AS SELECT qid,cid,cat,area_m2/1000000 sqkm FROM theme15;
-----------------------------------------------------------------------------------------------------------------------------
-- PROCESSING
-----------------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------
-- COUNTRY PROCESSING
-----------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS country_theme15;CREATE TEMPORARY TABLE country_theme15 AS
WITH
a AS (SELECT * FROM (SELECT DISTINCT country_id,is_marine,is_protected,qid,cid FROM country_index) a NATURAL JOIN theme15_sqkm),
--theme_stats
...
-- PA PROCESSING
-----------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS pa_theme15;CREATE TEMPORARY TABLE pa_theme15 AS
WITH
a AS (SELECT wdpaid ,cat,SUM(sqkm) theme_sqkm FROM (SELECT pa wdpaid,qid,cid FROM pa_index) a NATURAL JOIN theme15_sqkm GROUP BY wdpaid,cat),
...
CREATE TABLE results_202601_cep_out.country_intermediate_mspa_2015 AS
SELECT * FROM country_theme15;
/* -----------------------------------------------------------------------------------------------------------------------------
-- ECOREGION OUTPUT
-----------------------------------------------------------------------------------------------------------------------------
...
CREATE TABLE results_202601_cep_out.wdpa_intermediate_mspa_2015 AS
SELECT * FROM pa_theme15;
