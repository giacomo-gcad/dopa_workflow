...
OUTCSV_ROOT="cep_esa_lcc_1995_2020"
...
echo "now running r.stats in parallel on 648 CEP tiles, "${IN_RASTER1}" and "${IN_RASTER2}" using ${NCORES} threads"
...
psql ${dbpar2} -t -v vNAME="temp_lcc" -v vSCHEMA=${RESULTSCH} -v LC_DATE1="lc_1995" -v LC_DATE2="lc_2020" -f ./sql/create_table_rstats_lcc.sql
psql ${dbpar2} -t -c "\copy ${RESULTSCH}.temp_lcc FROM '${RESULTSPATH}/${FINALCSV}.csv' delimiter '|' csv"
psql ${dbpar2} -t -c "DROP TABLE IF EXISTS theme; CREATE TEMPORARY TABLE theme AS SELECT qid,cid,lc_1995,lc_2020,area_m2 FROM ${RESULTSCH}.temp_lcc;
...
c AS (SELECT ROW_NUMBER () OVER () cat,a.lc1_code lc1_1995,b.lc1_code lc1_2020 FROM b a CROSS JOIN b ORDER BY a.lc1_code,b.lc1_code),
d AS (SELECT qid,cid,lc_1995,a.lc1_code lc1_1995,lc_2020,b.lc1_code lc1_2020,area_m2 FROM theme JOIN a ON a.lc_code=lc_1995 JOIN a b ON b.lc_code=lc_2020),
e AS (SELECT * FROM d NATURAL JOIN c),
f AS (SELECT qid,cid,cat,lc1_1995,lc1_2020,SUM(area_m2) area_m2 FROM e GROUP BY qid,cid,cat,lc1_1995,lc1_2020 ORDER BY qid,cid,cat),
g AS (SELECT DISTINCT f.qid,f.cid,c.cat,c.lc1_1995,c.lc1_2020 FROM f,c)
SELECT * FROM g LEFT JOIN f USING(qid,cid,cat,lc1_1995,lc1_2020) WHERE cid!=0 ORDER BY qid,cid,cat;
"
