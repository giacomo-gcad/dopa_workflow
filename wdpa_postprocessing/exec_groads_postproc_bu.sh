...
for file in $(ls ${RESULTSPATH}/bu_groad*.csv | xargs -n 1 basename)
	do
	tab=${file%.csv}
...
psql ${dbpar} -t -v BU_SCHEMA=${bu_schema} -v WDPADATE=${wdpadate} -v SCHEMA_PRESSURES=${schema_pressures} -v SCHEMA_RESULTS=${schema_results} -f ./sql/groads_bu_postproc.sql
