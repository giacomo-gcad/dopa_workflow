...
for file in $(ls ${RESULTSPATH}/pa_groad*.csv | xargs -n 1 basename)
	do
	tab=${file%.csv}
...
psql ${dbpar} -t -v PA_SCHEMA=${pa_schema} -v WDPADATE=${wdpadate} -v SCHEMA_PRESSURES=${schema_pressures} -v SCHEMA_RESULTS=${schema_results} -f ./sql/groads_pa_postproc.sql
