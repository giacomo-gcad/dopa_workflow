#!/bin/bash
## IMPORT AND PROCESS IN POSTGRES RESULTS FOR WORLDCLIM
...
for file in $(ls ${RESULTSPATH}/pa_clim*.csv | xargs -n 1 basename)
	do
	tab=${file%.csv}
	fil=${file}
	sed -i 's/-1.79769313486232e+308/0/g' ${RESULTSPATH}/${file}
...
psql ${dbpar} -t -v PA_SCHEMA=${pa_schema} -v SCHEMA_CLIM=${schema_climate} -v DATE=${wdpadate} -f ./sql/worldclim_pa_postproc.sql
...
echo "---------------------------------------------------------------------------------------"
echo "Worldclim data post-processed in "${runtime}" seconds"
echo "---------------------------------------------------------------------------------------"
exit
