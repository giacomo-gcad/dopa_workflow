...
segments_table="segments_pa"
segments_attr_pa="segments_attr_pa"
segments_attr_pa_full_path=${RESULTSPATH}"/hdi/"${segments_attr_pa}
pa_schema="pa_202501"
...
psql ${dbpar} -t -v WDPA_SCHEMA=${wdpa_schema} -v WDPA_DATE=${wdpadate} -v LIST_PA=${pa_tc_list} -v SCHEMA_RESULTS=${schema_results} -v TABLE_IND="wdpa_habitat_diversity_profile_thdi_"${wdpadate} -v SCHEMA_HDI=${schema_hdi} -v PA_SCHEMA=${pa_schema} -v DATA_HDI=${segments_attr_pa}_${wdpadate} -f ./sql/thdi_pa_postproc.sql
...
${RESULTSPATH}"/hdi/"${segments_table}".shp"
